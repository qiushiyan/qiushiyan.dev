        ...
        <ul className="grid border-l px-2">
          {similarPosts.map((post) => (
            <li key={post.slug}>
              <Link
                className={cn(
                  "flex overflow-hidden rounded-md px-2 py-1 text-sm font-medium text-muted-foreground ring-ring transition-colors hover:text-primary/80 focus-visible:ring-2"
                )}
          ...
          {post.headings.map((item) => {
            ...
            return (
              <li
                ...
                className={cn(
                  "relative py-0.5",
                  item.depth === 3 ? "ml-2" : "px-0"
                )}
              >
                {isActive && (
                  <motion.div
                    layoutId={shouldReduceMotion ? undefined : "toc-indicator"}
                    className="absolute -left-px top-0 h-full w-0.5 bg-primary"
                    transition={{
                      type: "spring",
                      duration: 0.3,
                      bounce: 0.1,

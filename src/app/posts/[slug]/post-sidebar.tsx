...
export const TocItem = ({ slug, children, ...props }: TocItemProps) => {
  return (
    <Link
      href={`#${slug}`}
      className="flex min-w-8 items-center gap-2 overflow-hidden rounded-md px-2 py-1 text-sm font-medium text-muted-foreground ring-ring transition-all hover:bg-accent hover:text-accent-foreground focus-visible:ring-2"
      {...props}
    >
      {children}
    </Link>

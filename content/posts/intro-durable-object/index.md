...
request, a worker instance will be spawned close to the user’s
geographic location to serve the request with minimal latency.
...
- **Persistence**: Do can manage states as persistent disk storage. All
  workers can send messages to it, mutate the state, and get fast,
  consistent responses. When an object becomes idle, it will be garbage
...
`wrangler.toml` configuration file. Let’s generate a minimal worker
